var setting = {
    data: {
        simpleData: {
            enable: true,
            idKey: "menuId",
            pIdKey: "parentId",
            rootPId: -1
        },
        key: {
            url:"nourl"
        }
    }
};
var ztree;

var vm = new Vue({
    el:'#pmpapp',

    data:{
        showList: true,
        title: null,
        currSysCode:"",
        currSysName:"",

        menu:{
            parentName:null,
            parentId:0,
            type:1,
            orderNum:0,

            sysCode:null,
            sysName:null
        }
    },
    methods: {

        getMenu: function(menuId){
            //加载菜单树
            $.get(baseURL + "sys/menu/select?sysCode="+vm.currSysCode, function(r){
                ztree = $.fn.zTree.init($("#menuTree"), setting, r.data.menuList);
                var node = ztree.getNodeByParam("menuId", vm.menu.parentId);
                ztree.selectNode(node);

                vm.menu.parentName = node.name;
            })
        },

        add: function(){
            if(vm.currSysCode==null || vm.currSysCode==''){
                alert("请先下拉选择所属系统！");
                return;
            }

            vm.showList = false;
            vm.title = "新增";
            vm.menu = {parentName:null,parentId:0,type:1,orderNum:0,sysCode:vm.currSysCode};
            vm.getMenu();
        },

        update: function () {
            var menuId = getMenuId();
            if(menuId == null){
                return ;
            }

            $.get(baseURL + "sys/menu/info/"+menuId, function(r){
                vm.showList = false;
                vm.title = "修改";
                vm.menu = r.data.menu;

                vm.getMenu();

                vm.currSysCode=vm.menu.sysCode;
                vm.currSysName=vm.menu.sysName;
            });
        },

        del: function () {
            var menuId = getMenuId();
            if(menuId == null){
                return ;
            }

            confirm('确定要删除选中的记录？', function(){
                $.ajax({
                    type: "POST",
                    url: baseURL + "sys/menu/delete",
                    data: "menuId=" + menuId,
                    success: function(r){
                        if(r.code === 0){
                            alert('操作成功', function(){
                                vm.reload();
                            });
                        }else{
                            alert(r.msg);
                        }
                    }
                });
            });
        },

        saveOrUpdate: function () {
            if(vm.validator()){
                return ;
            }

            var url = vm.menu.menuId == null ? "sys/menu/save" : "sys/menu/update";
            $.ajax({
                type: "POST",
                url:  baseURL + url,
                contentType: "application/json",
                data: JSON.stringify(vm.menu),
                success: function(r){
                    if(r.code === 0){
                        alert('操作成功', function(){
                            vm.reload();
                        });
                    }else{
                        alert(r.msg);
                    }
                }
            });
        },

        menuTree: function(){
            layer.open({
                type: 1,
                offset: '30px',
                skin: 'layui-layer-molv',
                title: "选择菜单",
                area: ['300px', '300px'],
                shade: 0,
                shadeClose: false,
                content: jQuery("#menuLayer"),
                btn: ['确定', '取消'],
                btn1: function (index) {
                    var node = ztree.getSelectedNodes();
                    //选择上级菜单
                    vm.menu.parentId = node[0].menuId;
                    vm.menu.parentName = node[0].name;

                    layer.close(index);
                }
            });
        },

        reload: function () {
            vm.showList = true;
            Menu.table.refresh();
        },

        validator: function () {
            if(isBlank(vm.menu.name)){
                alert("菜单名称不能为空");
                return true;
            }

            //菜单
            if(vm.menu.type === 1 && isBlank(vm.menu.url)){
                alert("菜单链接url不能为空");
                return true;
            }
        },

        reset: function () {
            window.location.href=baseURL+"modules/sys/menu.html"
        },

    }
});


var Menu = {
    id: "menuTable",
    table: null,
    layerIndex: -1
};

/**
 * 初始化表格的列
 */
Menu.initColumn = function () {
    var columns = [
        {field: 'selectItem', radio: true},
        {title: '菜单Id', field: 'menuId', visible: false, align: 'center', valign: 'middle', width: '80px'},
        {title: '菜单名称', field: 'name', align: 'center', valign: 'middle', sortable: true, width: '180px'},
        {title: '父级菜单', field: 'parentName', align: 'center', valign: 'middle', sortable: true, width: '100px'},
        {title: '图标', field: 'icon', align: 'center', valign: 'middle', sortable: true, width: '80px', formatter: function(item, index){
            return item.icon == null ? '' : '<i class="'+item.icon+' fa-lg"></i>';
        }},

        {title: '类型', field: 'type', align: 'center', valign: 'middle', sortable: true, width: '100px', formatter: function(item, index){
            if(item.type === 0){
                return '<span class="label label-primary">目录</span>';
            }
            if(item.type === 1){
                return '<span class="label label-success">菜单</span>';
            }
            if(item.type === 2){
                return '<span class="label label-warning">按钮</span>';
            }
        }},

        {title: '排序编号', field: 'orderNum', align: 'center', valign: 'middle', sortable: true, width: '100px'},
        {title: '菜单链接url', field: 'url', align: 'center', valign: 'middle', sortable: true, width: '160px'},
        {title: '授权编码', field: 'perms', align: 'center', valign: 'middle', sortable: true,width:'200px'},

        {title: '所属系统', field: 'sysName', align: 'center', valign: 'middle', sortable: true}
        ];
    return columns;
};


//下框框动态绑定数据
$(function () {
    $.get(baseURL + "sys/config/list", function(r){
        var dataList=r.data.list;
        for(var key in dataList){
            var data=dataList[key];

            $("#systemList").append("<option value='" + data.code + "'>" + data.name + "</option>");
        }
    });
    $("#systemList").prepend("<option value=''>请选择所属系统....</option>");
});

$('#systemList').change(function () {
    var val=$(this).children("option:selected").val();
    var text=$(this).children("option:selected").text();

    $("#currSysCode").val(val);
    vm.currSysCode=val;

    $("#currSysName").html(text);
    vm.currSysName=text;

    initMenuDatas(val);
});

$(function () {
    var currSysCode=$("#currSysCode").val();

    initMenuDatas(currSysCode);
});

function initMenuDatas(currSysCode){
    var colunms = Menu.initColumn();
    var table = new TreeTable(Menu.id, baseURL + "sys/menu/list?sysCode="+currSysCode, colunms);

    table.setExpandColumn(2);
    table.setIdField("menuId");
    table.setCodeField("menuId");
    table.setParentCodeField("parentId");
    table.setExpandAll(true);
    table.init();
    Menu.table = table;
}

//获取菜单记录主键id
function getMenuId () {
    var selected = $('#menuTable').bootstrapTreeTable('getSelections');
    if (selected.length == 0) {
        alert("请选择一条记录");
        return null;
    } else {
        return selected[0].id;
    }
}








